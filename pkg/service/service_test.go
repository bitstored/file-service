	...
	tests := []struct {
	...
	}{
		{
			...
			args: args{
				ctx:       context.Background(),
				userID:    "uid:1",
				fileID:    "flid:MschG2smp7yRW43j271+Jw",
				secretKey: []byte("ana"),
			},
			want: &pb.File{
				Content: []byte("ana are mere"),
...
func TestService_ComputeSize(t *testing.T) {
	type fields struct {
		Repo *repo.Repository
	}
	type args struct {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, got1, err := s.ComputeSize(tt.args.ctx, tt.args.userID)
			if (err != nil) != tt.wantErr {
				t.Errorf("Service.ComputeSize() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if got != tt.want {
				t.Errorf("Service.ComputeSize() got = %v, want %v", got, tt.want)
			}
			if got1 != tt.want1 {
				t.Errorf("Service.ComputeSize() got1 = %v, want %v", got1, tt.want1)
			}
		})
	}
}

func TestService_GetMyDriveId(t *testing.T) {
	type fields struct {
		Repo *repo.Repository
	}
	type args struct {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := s.GetMyDriveId(tt.args.ctx, tt.args.userID)
			if (err != nil) != tt.wantErr {
				t.Errorf("Service.GetMyDriveId() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if got != tt.want {
				t.Errorf("Service.GetMyDriveId() = %v, want %v", got, tt.want)
			}
		})
	}
}
